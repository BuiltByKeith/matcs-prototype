...
type NavItem = {
  ...
  label: string;
  icon: string;
  badge?: { kind: "alpha" | "new" | "count"; text: string };
};
...
function SidebarItem({ item, active }: { item: NavItem; active: boolean }) {
  return (
    <Link href={item.href} className={`sb-item ${active ? "active" : ""}`}>
      ...
      {item.badge && (
        <span className={`sb-badge ${item.badge.kind}`}>{item.badge.text}</span>
      )}
    </Link>
  );
}
          ...
          >
            ...
            <div className="sb-user">
              ...
              <div className="sb-user-role">
                Admin · aradillos@cloudshift.net
              </div>
            </div>
            <Icon
              name="chevD"
